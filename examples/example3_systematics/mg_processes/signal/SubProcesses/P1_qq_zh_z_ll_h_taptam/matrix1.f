...
C     
C     Process: u u~ > z h HIG<=1 HIW<=1 NP<=2 QCD=0 QED<=99 @1
C     *   Decay: z > mu+ mu- HIG<=1 HIW<=1 NP=0 QCD=0 QED<=99
C     *   Decay: h > ta+ ta- HIG<=1 HIW<=1 NP=0 QCD=0 QED<=99
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
C     
...
C     ----------
      IF (FIRST) THEN
        FIRST=.FALSE.
        FK_MDL_WTAU = SIGN(MAX(ABS(MDL_WTAU), ABS(MDL_MTA
     $   *SMALL_WIDTH_TREATMENT)), MDL_WTAU)
     ...
     $   , ZERO)
      ...
      CALL FFV2_4_3(W(1,3),W(1,4),GC_71,GC_80,MDL_MZ, FK_MDL_WZ,W(1,5))
      CALL IXXXXX(P(0,5),MDL_MTA,NHEL(5),-1*IC(5),W(1,4))
      CALL OXXXXX(P(0,6),MDL_MTA,NHEL(6),+1*IC(6),W(1,3))
      CALL FFS1_3(W(1,4),W(1,3),GC_143,MDL_MH, FK_MDL_WH,W(1,6))
      CALL FFV1P0_3(W(1,1),W(1,2),GC_4,ZERO, FK_ZERO,W(1,3))
C     Amplitude(s) for diagram number 1
      CALL VVS1_5_0(W(1,3),W(1,5),W(1,6),GC_125,GC_126,AMP(1))
      CALL FFV2_5_3(W(1,1),W(1,2),-GC_71,GC_79,MDL_MZ, FK_MDL_WZ,W(1,3)
     $ )
C     Amplitude(s) for diagram number 2
      CALL VVS3_0(W(1,5),W(1,3),W(1,6),GC_127,AMP(2))
C     Amplitude(s) for diagram number 3
      CALL VVS2_6_0(W(1,5),W(1,3),W(1,6),GC_121,GC_128,AMP(3))
